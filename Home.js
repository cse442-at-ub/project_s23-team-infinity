...
// Fetch event data from php server
 const fetchUserEvents = async () => {
     try {
	 //PHP server
    const response = await fetch('/xuanpuzh/event.php');
...
const deleteEventFromBackend = async (eventData) => {
    try {
	//PHP Server
    const response = await fetch('/xuanpuzh/deleteEvent.php', {
      method: 'POST',
      headers: {
...
//User Events
 setUserEvents((prevUserEvents) => [
    ...prevUserEvents,
    { ...newEvent, time: formattedTime, endTime: formattedEndTime, duration },
  ]);
};
...
const handleDeleteEvent = (date, eventToDelete) => {
  ...
  // Remove the event in state 
  setUserEvents((prevUserEvents) => {
    return prevUserEvents.filter(
      (event) =>
